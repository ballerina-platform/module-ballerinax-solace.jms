...
isolated function testSvcWithResourceMethods() returns error? {
    ...
    if result is Error {
        test:assertEquals(
                result.message(),
                "Failed to attach service to listener: Solace service cannot have resource methods.",
                "Invalid error message received");
    }
}
...
isolated function testSvcWithNoRemoteMethods() returns error? {
    ...
    if result is Error {
        test:assertEquals(
                result.message(),
                "Failed to attach service to listener: Solace service must have exactly one or two remote methods.",
                "Invalid error message received");
    }
}

...
# Solace SMF message publisher which publishes messages with direct (at-most-once) delivery.
# Use this publisher for high-throughput scenarios which can tolerate message loss.
# For guaranteed delivery, use the `smf:PersistentPublisher`.
public isolated client class DirectPublisher {

    # Initializes a new Solace SMF direct message publisher with the given broker URL and configuration.
    # ```ballerina
    # smf:DirectPublisher publisher = check new ("smf://localhost:55554", auth = {
    #     username: "admin",
    #     password: "admin"
    # });
    # ```
    ...
    isolated function externInit(string url, PublisherConfiguration config) returns Error? = @java:Method {
        name: "init",
        'class: "io.ballerina.lib.solace.smf.publisher.DirectPublisherActions"
    } external;

    # Publishes a message to the given topic with direct (at-most-once) delivery.
    # ```ballerina
    ...
    isolated remote function publish(anydata|Message message, string topic) returns Error? {
        ...
        return self.externPublish(toInternalMessage(smfMessage), topic);
    }

    isolated function externPublish(InternalMessage message, string topic) returns Error? = @java:Method {
        name: "publish",
        'class: "io.ballerina.lib.solace.smf.publisher.DirectPublisherActions"
    } external;
